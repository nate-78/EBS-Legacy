...
namespace ACAfiling.Controllers {
    ...
    public class UiNavigation {
        ...
        //static string chromeLoc = AppDomain.CurrentDomain.BaseDirectory;
        //static ChromeDriver driver = new ChromeDriver(chromeLoc);
        ...
        public static StatusUpdate NavigateUI(string manifestPath, string formdataPath, bool isProduction, bool isSubmission, 
            ...
            try {
                ...
                if (!isProduction) {
                    url = ConfigurationManager.AppSettings["UiUriTest"].ToString();
                }
                ...
                using (driver) {
                    ...
                    var success = loginAca(user, pass, url);

                    // only proceed if logged in
                    if (success) {
                        if (!isProduction) { // this check may need to be done elsewhere now that the login pages have changed (01/2018)
                            handleTestingPage();
                        }
        ...
        private static void handleTestingPage() {
            ...
            if (elementExists(By.XPath("//input[@name='aatsVersion'][position()=1]")) && elementExists(By.XPath("//input[@value='Submit Selected Version']"))) {
                driver.FindElement(By.XPath("//input[@name='aatsVersion'][position()=1]")).Click();
                driver.FindElementByXPath("//input[@value='Submit Selected Version']").Click();
            }
        }
        ...
        // REFACTORING SOME FUNCTIONALITY IN JANUARY 2018 TO ACCOUNT FOR NEW
        // ACA UI LOGIN PROCESS
        private static bool loginAca(string user, string pass, string url) {
            bool result = false;
            ...
            // try to find the username and submit inputs.  Assign them to the above 
            //      variables, if they exist
            while ((userNameField == null  || loginButton == null) && attempts < 5) {
                ...
                if (elementExists(By.Id("submit"))) {
                    loginButton = driver.FindElementById("submit");
                }
            ...
            if (userNameField != null || loginButton != null) {
                ...
                // complete the steps in pages 2 - 4. If all are successful, and access has not been denied,
                //      then return 'true' so the application can proceed 
                if (loginAcaPage2(pass) && loginAcaPage3() && loginAcaPage4() && !isAccessDenied()) {
                    result = true;
                }
            }
        ...
        // handle the 2nd page of the login process
        //      this is the 'site image' and 'site phrase' part of the login process.
        //      We are going to just enter the password and move on. If necessary,
        //      we could always add in checks for the values, but since the user doesn't
        //      actually have to do anything with them, we're going to ignore them for now.
        private static bool loginAcaPage2(string password) {
            // see if password expiration warning shows up
            handlePwordExpiringPage(); // this is older functionality that I assume still works...

            bool result = false;
            IWebElement passField = null;
            ...
            if (elementExists(By.Id("submit"))) {
                submitBtn = driver.FindElementById("submit");
            }

            if (passField != null && submitBtn != null) {
        ...
        // handle the 3rd page of the login process
        //      this page is kind of like an intro page.  Just shows last time logged in, etc
        private static bool loginAcaPage3() {
            // see if password expiration warning shows up
            handlePwordExpiringPage(); // this is older functionality that I assume still works...

            var result = false;
            if (elementExists(By.Name("submit"))) {
        ...
        // handle the 4th page of the login process
        //      select if logging in as individual or as representative of EBS
        private static bool loginAcaPage4() {
            // see if password expiration warning shows up
            handlePwordExpiringPage(); // this is older functionality that I assume still works... 

            var result = false;
            if (elementExists(By.Id("USERNAME1")) && elementExists(By.Id("submit"))) {
                driver.FindElementById("USERNAME1").Click();
        ...
        private static bool isAccessDenied() {
            if (driver.Title.ToLower().Contains("access denied")) {
                return true;
            } else {
                return false;
            }
