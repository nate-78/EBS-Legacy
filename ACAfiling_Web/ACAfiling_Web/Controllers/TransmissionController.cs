...
namespace ACAfiling_Web.Controllers {
    ...
    public class TransmissionController {
        ...
        public static List<CustomResponse> SubmitAcaRecord(Record rec, CorrectionInfo correctionInfo) {
            ...
            while (rec.HasRecordsToSubmit) { // as long as there are records to submit, keep doing it. Will usually just be one submission, though
                ...
                byte[] blob = XmlController.BuildXmlDoc(rec, correctionInfo);

                // get checksum on the form data file
                string checkSum = X509CertSecurityManager.ComputeCheckSum(blob);
                ...
                if (update.StatusType != "Error" && !String.IsNullOrEmpty(update.ReceiptId)) {
                    ...
                    try {
                        var currUser = ACAfiling_Web.Models.CurrentUserSingleton.GetUser();
        ...
        public static StatusUpdate GetStatusUpdate(string receiptId) {
            ...
            // get the TCC
            string tcc = ConfigurationManager.AppSettings["TCC"];
            if (ConfigurationManager.AppSettings["TestingOrProduction"] == "T") { // use testing TCC
                tcc = ConfigurationManager.AppSettings["TCC_Test"];
            }
            busHeader.UniqueTransmissionId = ReportBuilder.CreateUniqueId(tcc);
