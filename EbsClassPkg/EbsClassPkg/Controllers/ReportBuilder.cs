...
namespace EbsClassPkg.Controllers {
    ...
    public class ReportBuilder {
        ...
        // used when creating a submission record from a Cloud File (WebJob)
        ...
        // used when creating a submission from a Web project
        public static Record CreateSubmissionRecord(string uploadedFilePath, string subType, CorrectionInfo correctionInfo) {
            Record record = new Record();

            // create FileInfo object from filepath, then create excel package from FileInfo
            FileInfo uploadedFile = new FileInfo(uploadedFilePath);
            using (ExcelPackage pkg = new ExcelPackage(uploadedFile)) {
                // get the workbook
                ExcelWorkbook workbook = pkg.Workbook;
                ...
                if (workbook.Worksheets.Count > 0) {
                    ExcelWorksheet ws1094 = workbook.Worksheets[1]; // index is 1-based, not 0-based
                    record = createRecord(ws1094, subType, uploadedFilePath);
        ...
        private static Record createRecord(ExcelWorksheet worksheet, string subType, string filePath = "") {
            ...
            // TCC
            if (ConfigurationManager.AppSettings["TestingOrProduction"] == "P") {
                record.TCC = ConfigurationManager.AppSettings["TCC"];
            } else {
                record.TCC = ConfigurationManager.AppSettings["TCC_Test"];
            }
            ...
            record.IrsFileName = "1094C_Request_" + record.TCC + "_" + 
            ...
            record.UniqueId = CreateUniqueId(record.TCC); // do this even for C and R submissions
        ...
        private static Record addIndividual1095cRecords(Record record, ExcelWorksheet worksheet) {
            ...
            while (!String.IsNullOrEmpty(FormatChecker.NullToString(worksheet2.Cells[row, 1], true))) {
                ...
                // I believe the indicator is in column 398 (OH)
                if (!String.IsNullOrEmpty(FormatChecker.NullToString(worksheet2.Cells[row, 398], true)) &&
                    FormatChecker.NullToString(worksheet2.Cells[row, 398], true).Trim().ToLower() == "yes") {
        ...
        private static IndividualReport build1095cReport(ExcelWorksheet wsheet, Int32 row) {
            ...
            // add employee age on Jan 1st (new for TY 2020)
            report.EmplyeeAgeOnJan1 = FormatChecker.NullToString(wsheet.Cells[row, 20], false);

            var planStartMt = FormatChecker.NullToString(wsheet.Cells[row, 21], false);
            ...
            report.CoverageCodeYr = FormatChecker.NullToString(wsheet.Cells[row, 22], true);
            report.CoverageCodeJan = FormatChecker.NullToString(wsheet.Cells[row, 23], true);
            report.CoverageCodeFeb = FormatChecker.NullToString(wsheet.Cells[row, 24], true);
            report.CoverageCodeMar = FormatChecker.NullToString(wsheet.Cells[row, 25], true);
            report.CoverageCodeApr = FormatChecker.NullToString(wsheet.Cells[row, 26], true);
            report.CoverageCodeMay = FormatChecker.NullToString(wsheet.Cells[row, 27], true);
            report.CoverageCodeJun = FormatChecker.NullToString(wsheet.Cells[row, 28], true);
            report.CoverageCodeJul = FormatChecker.NullToString(wsheet.Cells[row, 29], true);
            report.CoverageCodeAug = FormatChecker.NullToString(wsheet.Cells[row, 30], true);
            report.CoverageCodeSep = FormatChecker.NullToString(wsheet.Cells[row, 31], true);
            report.CoverageCodeOct = FormatChecker.NullToString(wsheet.Cells[row, 32], true);
            report.CoverageCodeNov = FormatChecker.NullToString(wsheet.Cells[row, 33], true);
            report.CoverageCodeDec = FormatChecker.NullToString(wsheet.Cells[row, 34], true);

            // low cost
            report.LowCostYr = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 35], false), row, 35);
            report.LowCostJan = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 36], false), row, 36);
            report.LowCostFeb = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 37], false), row, 37);
            report.LowCostMar = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 38], false), row, 38);
            report.LowCostApr = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 39], false), row, 39);
            report.LowCostMay = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 40], false), row, 40);
            report.LowCostJun = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 41], false), row, 41);
            report.LowCostJul = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 42], false), row, 42);
            report.LowCostAug = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 43], false), row, 43);
            report.LowCostSep = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 44], false), row, 44);
            report.LowCostOct = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 45], false), row, 45);
            report.LowCostNov = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 46], false), row, 46);
            report.LowCostDec = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 47], false), row, 47);

            // safe harbor codes
            report.SafeHarborYr = FormatChecker.NullToString(wsheet.Cells[row, 48], true);
            report.SafeHarborJan = FormatChecker.NullToString(wsheet.Cells[row, 49], true);
            report.SafeHarborFeb = FormatChecker.NullToString(wsheet.Cells[row, 50], true);
            report.SafeHarborMar = FormatChecker.NullToString(wsheet.Cells[row, 51], true);
            report.SafeHarborApr = FormatChecker.NullToString(wsheet.Cells[row, 52], true);
            report.SafeHarborMay = FormatChecker.NullToString(wsheet.Cells[row, 53], true);
            report.SafeHarborJun = FormatChecker.NullToString(wsheet.Cells[row, 54], true);
            report.SafeHarborJul = FormatChecker.NullToString(wsheet.Cells[row, 55], true);
            report.SafeHarborAug = FormatChecker.NullToString(wsheet.Cells[row, 56], true);
            report.SafeHarborSep = FormatChecker.NullToString(wsheet.Cells[row, 57], true);
            report.SafeHarborOct = FormatChecker.NullToString(wsheet.Cells[row, 58], true);
            report.SafeHarborNov = FormatChecker.NullToString(wsheet.Cells[row, 59], true);
            report.SafeHarborDec = FormatChecker.NullToString(wsheet.Cells[row, 60], true);

            // ICHRA Zip codes (new for TY 2020)
            report.IchraZipCdYr = FormatChecker.NullToString(wsheet.Cells[row, 61], true);
            report.IchraZipCdJan = FormatChecker.NullToString(wsheet.Cells[row, 62], true);
            report.IchraZipCdFeb = FormatChecker.NullToString(wsheet.Cells[row, 63], true);
            report.IchraZipCdMar = FormatChecker.NullToString(wsheet.Cells[row, 64], true);
            report.IchraZipCdApr = FormatChecker.NullToString(wsheet.Cells[row, 65], true);
            report.IchraZipCdMay = FormatChecker.NullToString(wsheet.Cells[row, 66], true);
            report.IchraZipCdJun = FormatChecker.NullToString(wsheet.Cells[row, 67], true);
            report.IchraZipCdJul = FormatChecker.NullToString(wsheet.Cells[row, 68], true);
            report.IchraZipCdAug = FormatChecker.NullToString(wsheet.Cells[row, 69], true);
            report.IchraZipCdSep = FormatChecker.NullToString(wsheet.Cells[row, 70], true);
            report.IchraZipCdOct = FormatChecker.NullToString(wsheet.Cells[row, 71], true);
            report.IchraZipCdNov = FormatChecker.NullToString(wsheet.Cells[row, 72], true);
            report.IchraZipCdDec = FormatChecker.NullToString(wsheet.Cells[row, 73], true);

            // employer provided self-insured (not sure what that means)
            report.EmployerProvidedSelfInsured = false;
            if (!String.IsNullOrEmpty(FormatChecker.NullToString(wsheet.Cells[row, 74], true))) {
            ...
            report = addCoveredIndividuals(report, wsheet, row, 75);
        ...
        public static string CreateUniqueId(string tcc) {
            string id = Guid.NewGuid().ToString() + ":SYS12:";
            // bring in TCC
            id += tcc + "::T"; // "T" is for "transactional"
        ...
        private static Record addOldRecordIds(Record rec, CorrectionInfo correctionInfo) {
            ...
            if (rec.SubmissionType == "C" && correctionInfo.FormType == FormType.Form1095) {
                // get old records
                List<IndividualReport> oldIndReports = AcaDataManager.GetRecordIdsByReceiptId(correctionInfo.BadSubmissionReceiptId);
                ...
                foreach (IndividualReport r in rec.IndividualReports) {
                    ...
                    if (result != null && result.Count() > 0) {
                        ...
                        var num = 0;
                        if (Int32.TryParse(result.First().Trim(), out num))
                        {
                            matchingRecordIds.Add(Convert.ToInt32(num));
                        }
        ...
        // pulled this function from TransportationController. Not sure if it can be removed from there in its entirety yet
