...
namespace PdfCreatorWebJob {
    ...
    //*******************************************************************************************************************
    // UPDATE: TRIED TO FIX THE ABOVE PROBLEM, BUT COULDN'T GET IT TO WORK IN THE EbsClassPkg ReportBuilder CLASS FOR 
    // SOME REASON. STILL HAVING TO RELY ON THIS FUNCTION
    //*******************************************************************************************************************
    ...
    public class PdfReportBuilder {
        ...
        private static Record addIndividual1095cRecords(Record record, ExcelWorksheet worksheet) {
            ...
            while (!String.IsNullOrEmpty(FormatChecker.NullToString(worksheet2.Cells[row, 1], true))) {
                ...
                // I believe the indicator is in column 400 (OH)
        ...
        private static IndividualReport build1095cReport(ExcelWorksheet wsheet, Int32 row) {
            ...
            // low cost
            report.LowCostYr = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 35], false), row, 34);
            report.LowCostJan = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 36], false), row, 35);
            report.LowCostFeb = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 37], false), row, 36);
            report.LowCostMar = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 38], false), row, 37);
            report.LowCostApr = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 39], false), row, 38);
            report.LowCostMay = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 40], false), row, 39);
            report.LowCostJun = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 41], false), row, 40);
            report.LowCostJul = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 42], false), row, 41);
            report.LowCostAug = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 43], false), row, 42);
            report.LowCostSep = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 44], false), row, 43);
            report.LowCostOct = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 45], false), row, 44);
            report.LowCostNov = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 46], false), row, 45);
            report.LowCostDec = FormatChecker.ReturnFloat(FormatChecker.NullToString(wsheet.Cells[row, 47], false), row, 46);
            ...
            // THIS ONLY APPLIES TO THE FormBuilder
            // set Corrected or Voided forms as needed
            report.IsCorrectedForm = false;
            report.IsVoidedForm = false;
            // column 405 is column OO -- but in 2020 this has changed to Column OJ (400)
            int voidColumnNumber = 400;
            if (!String.IsNullOrEmpty(FormatChecker.NullToString(wsheet.Cells[row, voidColumnNumber], true)) &&
                    FormatChecker.NullToString(wsheet.Cells[row, voidColumnNumber], true).Trim().ToLower() == "c") {
                report.IsCorrectedForm = true;
            }
            if (!String.IsNullOrEmpty(FormatChecker.NullToString(wsheet.Cells[row, voidColumnNumber], true)) &&
                    FormatChecker.NullToString(wsheet.Cells[row, voidColumnNumber], true).Trim().ToLower() == "v") {
                report.IsVoidedForm = true;
            }
